...
void CSoundRender_CoreA::_initialize(u64 window)
{
	...
	Listener.velocity.set(0.0f, 0.0f, 0.0f);
...
void CSoundRender_CoreA::update_listener(const Fvector& P, const Fvector& D, const Fvector& N, float dt)
{
	...
	Listener.velocity.sub(P, Listener.position);
	Listener.velocity.div(dt);
	...
	A_CHK(alListener3f(AL_VELOCITY, Listener.velocity.x, Listener.velocity.y, -Listener.velocity.z));
